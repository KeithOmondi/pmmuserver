...
const categories: SeedCategory[] = [
  ...
  {
    code: "A.1",
    title: "Core Service Delivery Initiatives",
    parentCode: "A",
    level: 2,
  },
  ...
  },
  {
    code: "A.1.2.b",
    title:
      "Prepare and submit annual status report on High Court Special Benches",
    parentCode: "A.1.2",
    level: 4,
  },
  {
  ...
  },
  {
    code: "A.1.15.a",
    title:
      "Prepare & submit quarterly performance reports by 5th of next month",
    parentCode: "A.1.15",
    level: 4,
  },
...
(async (): Promise<void> => {
  try {
    // ✅ Only check MONGO_URI
    if (!env.MONGO_URI) {
      throw new Error("❌ MONGO_URI is missing");
    }

    // ✅ Connect using only the URI (DB name is inside the URI)
    ...
    for (const cat of categories) {
      ...
      await newCategory.save(); // ✅ no overload issues
